...
SELECT 
    ...
    COUNT(DISTINCT "KEY") FILTER (WHERE behavior IN ('Engagement', 'Curiosity & Critical Thinking', 'Curiosity & Critical Thinking and Engagement') AND forms IN ('cro_ug', 'cro', 'cro_indo') AND subindicator IN ('e1', 'e2', 'c1') AND score IS NOT NULL) AS engagement_count,
    COUNT(DISTINCT "KEY") FILTER (WHERE behavior IN ('Curiosity & Critical Thinking', 'Curiosity & Critical Thinking and Engagement') AND forms IN ('cro_ug', 'cro', 'cro_indo') AND score IS NOT NULL) AS curiosity_critical_thinking_count,
